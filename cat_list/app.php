...
$Flag=["0"];
   ...
   $ibo= $row['IBO'];
	...
	else if($_GET['name']=='fbgmregister'){
...
$Username=$_POST['Username'];
    ...
    // Look up the last 10 visits
   $select = $pdo->prepare(
'SELECT Username FROM distributor_profile_hdr where Username=:username1 and Flag=:Flag1' );
$select->execute(array(':username1'=>$Username,':Flag1'=>$Flag));
$visits = [""];
while ($row = $select->fetch(PDO::FETCH_ASSOC)) {
 $Username= $row['Username'];
...
$format = strtolower($_GET['format']) == 'json'; //xml is the default
    // Look up the last 10 visits
   $select = $pdo->prepare(
        'insert into distributor_profile_hdr(Username,Sponsor,Created_Date,Flag) values(:Username1,'1000',now(),'1')');
    $select->execute(array(':Username1'=>$Username));
    $visits = [""];
    while ($row = $select->fetch(PDO::FETCH_ASSOC)) {
 $Username= $row['Username'];	
    }
